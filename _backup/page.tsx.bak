...
export default function BookingPage() {
  ...
  const fetchAvailability = useCallback(async (month: Date) => {
    ...
    try {
      ...
      const to = format(endOfMonth(addDays(month, 40)), 'yyyy-MM-dd') // fetch two months at once
          ...
          <form onSubmit={handleSubmit}>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
              ...
              <div className="bg-brand-50 rounded-xl p-4 text-sm text-gray-700 space-y-1">
                ...
                <p>
                  <span className="font-medium">Personen:</span> {form.total_people} (
                  {form.children_count} kinderen)
                </p>
                <p>
