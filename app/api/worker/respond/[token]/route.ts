...
export async function POST(
  ...
  try {
    ...
    // Fetch the response row with booking + worker details
    ...
    // If already handled, return friendly message
    ...
    // If booking is already approved by someone else
    ...
    if (action === 'accept') {
      ...
      // Notify other workers that slot is taken
      ...
      if (otherResponses?.length) {
        ...
        // Mark their responses as declined
        await supabaseAdmin
          .from('booking_responses')
    ...
    if (allDeclined) {
      // Mark booking as denied and notify visitor
      await supabaseAdmin
        .from('bookings')
        .update({
          status: 'denied',
