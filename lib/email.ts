...
function mapsButton(siteUrl: string): string {
  // Use absolute URL so the icon loads in email clients
  const iconUrl = `${siteUrl}/icons/googlemaps.svg`
  return `<a href="${MAPS_URL}" style="display:inline-block;padding:10px 18px;background:#fff;color:#1a73e8;text-decoration:none;border-radius:10px;font-weight:600;font-size:14px;border:2px solid #1a73e8;vertical-align:middle;line-height:1"><img src="${iconUrl}" width="16" height="16" alt="" style="vertical-align:middle;margin-right:7px;margin-bottom:1px">Bekijk op Google Maps</a>`
}
